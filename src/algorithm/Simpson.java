...
public class Simpson {
    ...
    public Simpson(BigDecimal a, BigDecimal b, BigDecimal accuracy, int functionNumber){
        this.a = a;
        this.b = b;
    ...
    public ArrayList<ArrayList<BigDecimal>> solve(){
        ...
        while ((module.compareTo(accuracy) > 0) || (iterationCount < 3)){
            ...
            for (int i = 0; i < answer.get(1).size(); i++){
                if ((i == 0) || (i == (answer.get(1).size() - 1))){
                    second = second.add(answer.get(1).get(i).multiply(h).multiply(new BigDecimal(1.0 / 3.0)));
                    answer.get(2).add(answer.get(1).get(i).multiply(h).multiply(new BigDecimal(1.0 / 3.0)));
                }
                else if (i % 2 == 1){
                    second = second.add(answer.get(1).get(i).multiply(h).multiply(new BigDecimal(4.0 / 3.0)));
                    answer.get(2).add(answer.get(1).get(i).multiply(h).multiply(new BigDecimal(4.0 / 3.0)));
                }
                else if (i % 2 == 0){
                    second = second.add(answer.get(1).get(i).multiply(h).multiply(new BigDecimal(2.0 / 3.0)));
                    answer.get(2).add(answer.get(1).get(i).multiply(h).multiply(new BigDecimal(2.0 / 3.0)));
                }
            }
